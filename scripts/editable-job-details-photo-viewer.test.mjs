...
test('photo viewer uses selected attachment Blob URL instead of window open or direct data img', () => {
  ...
  assert.match(appSource, /const \[imageUrl\] = useState\(\(\) => attachmentToObjectUrl\(attachment\)\)/)
  assert.match(appSource, /import type \{ AttachmentPreviewState \} from '\.\/attachmentUtils'/)
  assert.match(appSource, /const \[previewState, setPreviewState\] = useState<AttachmentPreviewState>\(\(\) => \(imageUrl \? 'loading' : 'error'\)\)/)
  assert.match(appSource, /if \(imageUrl\) URL\.revokeObjectURL\(imageUrl\)/)
