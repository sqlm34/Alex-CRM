...
test('photo viewer uses selected attachment Blob URL instead of window open or direct data img', () => {
  ...
  assert.match(appSource, /const \{ url: imageUrl, error: imageError \} = useLegacyImage\(attachment\)/)
  assert.match(appSource, /await compatibleImageBlob\(blob, attachment.filename\)/)
  assert.match(appSource, /import type \{ AttachmentPreviewState \} from '\.\/attachmentUtils'/)
  assert.match(appSource, /const previewState = imageError \? 'error' : imageState/)
