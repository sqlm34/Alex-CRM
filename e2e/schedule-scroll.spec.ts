...
for (const viewport of [{ width: 390, height: 844 }, { width: 360, height: 740 }, { width: 1280, height: 900 }]) {
  test(`full schedule opens today without trailing spacer ${viewport.width}`, async ({ page }) => {
    ...
    if (viewport.width === 390) {
      ...
      const reviews = page.getByRole('button', { name: 'Reviews', exact: true })
      await reviews.click()
      await expect(page.locator('.job-eta-message')).toHaveText("Schedule, I would really appreciate it if you would leave a review in my google business account. Thank you!!!\nhttps://g.page/r/CSFvvmB61pviEAE/review\nThank you in advance. Have a good day.")
      const etaBox = await page.getByRole('button', { name: 'TEXT ETA', exact: true }).boundingBox()
      const reviewsBox = await reviews.boundingBox()
      expect(reviewsBox!.y).toBe(etaBox!.y)
      expect(reviewsBox!.x).toBeGreaterThan(etaBox!.x)
      await page.screenshot({ path: 'test-results/review-request-mobile.png' })
    }
    await page.reload()
