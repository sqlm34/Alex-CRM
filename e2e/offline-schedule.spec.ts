...
test('real CRM opens today and offline item payment keeps Void opposite amount', async ({ page }) => {
  ...
  await expect(page.locator('.schedule-day-group').first()).toHaveAttribute('data-today', 'true')
  await expect(page.getByRole('button', { name: /Past customer/ })).toHaveCount(0)
  await page.getByRole('button', { name: 'Earlier jobs', exact: true }).click()
  await expect(page.getByRole('button', { name: /Past customer/ })).toBeVisible()
  await page.getByRole('button', { name: 'Today and upcoming', exact: true }).click()
  await expect(page.locator('.schedule-day-group').first()).toHaveAttribute('data-today', 'true')
